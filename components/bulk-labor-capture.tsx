...
export function BulkLaborCapture({ isOpen, onClose, onSuccess }: Props) {
    ...
    const handlePaste = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
        ...
        const parsed: ParsedItem[] = lines.map(line => {
            ...
            return {
                ...
                unidad: cols[2]?.trim() || "",
                salarioBase: cols[3]?.trim()?.replace(/[$,]/g, "") || "0",
                factorSalarioReal: cols[4]?.trim() || "1.0",
                status: 'pending'
            }
        })
        setItems(parsed)
    ...
    return (
        <Dialog isOpen={isOpen} onClose={onClose} title="Captura Masiva de Mano de Obra" maxWidth="2xl">
                ...
                {!items.length ? (
                    <div className="flex-1 flex flex-col gap-4">
                        <div className="p-4 rounded-xl bg-blue-50 border border-blue-100 text-blue-800 text-xs leading-relaxed">
                            ...
                            <p>Copia una tabla de Excel con las columnas: <b>Clave, Descripción, Unidad, Salario Base, FASAR</b>. Pégala en el recuadro de abajo.</p>
                        </div>
                        <Textarea
                    ...
                    <div className="flex-1 flex flex-col overflow-hidden">
                        <div className="flex items-center justify-between mb-4">
                            <p className="text-xs text-slate-500 font-medium">Se detectaron <span className="text-primary font-bold">{items.length}</span> registros.</p>
                                ...
                                <tbody>
                                    {items.map((item, idx) => (
                                        <tr key={idx} className="border-b last:border-0 hover:bg-slate-50 transition-colors">
                                            <td className="px-3 py-1.5 font-mono text-blue-600 font-bold">{item.clave}</td>
                                            <td className="px-3 py-1.5 truncate max-w-[200px] text-slate-600">{item.descripcion}</td>
                                            <td className="px-3 py-1.5 text-slate-500">${item.salarioBase}</td>
                        ...
                        <div className="pt-6 flex justify-end gap-3 mt-auto">
                            ...
                            <Button
                                ...
                                className="px-8 bg-blue-600 hover:bg-blue-700 text-white rounded-xl shadow-lg shadow-blue-500/20 font-bold min-w-[140px]"
                            >
                                {isProcessing ? (
                                    <>
